...
def doesArmTouchObjects(armPosDist, objects, isGoal=False):
    ...
    """
    #referenced mathimatical equations from https://stackoverflow.com/questions/1073336/circle-line-segment-collision-detection-algorithm
    if (isGoal == False):
        for link in armPosDist:
            start = np.array(link[0])
            end = np.array(link[1])
            pad = link[2]
            for obj in objects:
                d = end - start
                circle = np.array([obj[0], obj[1]])
                f = start - circle
                a = np.dot(d, d)
                b = 2* np.dot(f, d)
                c = np.dot(f, f) - math.pow((obj[2]+pad), 2)
                discrim = math.pow(b, 2) - 4*a*c
                if (discrim >= 0):
                    discrim = math.sqrt(discrim)
                    sol1 = (-b + discrim) / (2*a)
                    sol2 = (-b - discrim) / (2*a)
                    if (sol1 >= 0 and sol1 <= 1):
                        #print("True")
                        return True
                    if (sol2 >= 0 and sol2 <= 1):
                        #print("True")
                        return True
        #print("False")
        return False
