...
class TabQPolicy(QPolicy):
    ...
    def qvals(self, states):
        ...
        """
        qval = []
        model_in = self.discretize(states[0])
        qval.append(self.model[model_in])
        return qval
    ...
    def td_step(self, state, action, reward, next_state, done):
        ...
        q = self.model[discrete][action]
        if(done == True and next_state[0] >= 0.5):
            reward = 1.0
            target = 1.0
        else:
            target = reward + self.gamma * max(self.model[next_discrete])
        updateq = q + self.lr * (target - q)
        self.model[discrete][action] = updateq
        ret = math.pow(q - target, 2)
        return ret
